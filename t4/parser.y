...
func-decl-list:
  func-decl-list func-decl                  { $$ = $1; add_child($1, $2); Tree* header = get_child($2, 0); int index = get_index(header); set_func_address(FuncT, index, $2); }
| func-decl                                 { $$ = new_subtree(FUNC_DECL_LIST_NODE, 1, $1); Tree* header = get_child($1, 0); int index = get_index(header); set_func_address(FuncT, index, $1); }

func-decl:
  func-header func-body                     { $$ = parent = new_subtree(FUNC_DECL_NODE, 2, $1, $2); scope++;}

func-header:
  ret-type ID LPAREN params RPAREN          { $$ = new_subtree(FUNC_HEADER_NODE, 3, $1, $2, $4); int index = get_index($2); int params_size = get_children_size($4); index = decl_function (index, params_size, yylineno); set_type($2, "id"); set_index($2, index); set_index($$, index);}

func-body:
  LBRACE opt-var-decl opt-stmt-list RBRACE   { $$ = new_subtree(FUNC_BODY_NODE, 2, $2, $3); }

opt-var-decl:
  %empty                                     {  $$ = new_subtree(VAR_DECL_LIST_NODE, 0); }
| var-decl-list                              {  $$ = $1; }
...
user-func-call:
  ID LPAREN opt-arg-list RPAREN        { $$ = new_subtree(USER_FUNC_CALL_NODE, 1, $3); int index = get_index($1); index = use_function(index, get_children_size($3), yylineno); set_index($1, index); set_index($$, index); }
