...
#define LITERAL_MAX_SIZE 256
#define LITERALS_TABLE_MAX_SIZE 256
...
#define SYMBOL_TABLE_MAX_SIZE 256
...
int set_var_memoffset(SymTable* st, int i, int offset){
        return st->t[i].offset = offset;
}
int get_var_memoffset(SymTable* st, int i){
...
void print_sym_table(SymTable* st) {
        ...
        for (int i = 0; i < st->size; i++) {
                printf("Entry %d -- name: %s, line: %d, scope: %d, offset:%d\n", i, get_var_name(st, i), get_var_line(st, i), get_var_scope(st, i), get_var_memoffset(st, i));
        }
}
...
void set_func_address(FuncTable* st, int i, Tree* ast_node){
        st->t[i].ast_node = ast_node;
}

Tree* get_func_address(FuncTable* st, int i){
        return st->t[i].ast_node;
}

void print_func_table(FuncTable* st) {
        printf("Functions table:\n");
        for (int i = 0; i < st->size; i++) {
                printf("Entry %d -- name: %s, line: %d, arity: %d, addr: %p\n", i, get_func_name(st, i), get_func_line(st, i), get_func_args(st, i),  get_func_address(st, i));
        }
}
