...
// Set the variable offset in the auxiliary memory in which is stored its value.
// No check is made by this function, so make sure that the index is valid first.
int set_var_memoffset(SymTable* st, int i, int offset);

// Returns the variable offset in the auxiliary memory in which is stored its value.
// No check is made by this function, so make sure that the index is valid first.
...
// Returns the address of the ast node.
// No check is made by this function, so make sure that the index is valid first.
void set_func_address(FuncTable* st, int i, Tree* address);
